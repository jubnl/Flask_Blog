...
def page_not_found(e):
    return render_template('404.html', title="404", sidebar=True), 404
...
@app.route('/')
def home():
    return render_template('home.html',posts=posts, title="Home", sidebar=True)
...
def register():
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        if query is not True:
            ...
            log = Model("t_logs")
            sql, res = log.insert_into_single_record(fields_values=insert_log, return_sql_query=True)
            print(sql)
...
@app.route("/admin/log/<int:id>", methods=['POST', 'GET'])
@is_user_logged_in
@is_admin
def log(id):
    model = Model("t_logs")
